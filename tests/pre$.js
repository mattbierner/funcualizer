...
describe('pre$', () => {
    it('should lookup and invoke differnt arity functions', () => {
        ...
        for (let i = 0 ; i < 8; ++i) {
            ...
            const f = functionizer.pre$(method, i);

            assert.strictEqual(i + 1, f.length);
            assert.deepEqual([3].concat(args.slice(0, i)), f.apply(null, [{ x: 3 }].concat(a)));
            assert.deepEqual([5].concat(args.slice(0, i)), f.apply(null, [{ x: 5 }].concat(a)));
        }
    });
});
